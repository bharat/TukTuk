...
class CurlUp: Animation {
    var title: String = "Curl Up"
    ...
    func animate(view: UIView, completion: @escaping ()->()) {
        ...
        let txs = [
            ...
            tx(from: views[7], to: views[8], duration: 0.50, delay: 0.75),
        ]
        transition(txs)
    ...
    func transition(_ txs: [tx]) {
        ...
        UIView.transition(from: tx.from, to: tx.to, duration: tx.duration, options: [.transitionCurlUp]) { _ in
